...
describe("ContractsService", () => {
  it("create throws NotFoundError when deal is missing", async () => {
    const crmQuery = createQuery(null);
    crmQuery.maybeSingle.mockResolvedValue({ data: { schema_name: "crm1" }, error: null });
    const idxQuery = createQuery({ data: null, error: null });
    ...
    const admin = {
      schema: jest.fn((schemaName: string) => ({
        from: jest.fn((table: string) => {
          ...
          if (schemaName === "core" && table === "contracts") return createQuery({ data: [], error: null });
    ...
    const service = new ContractsService(supabase, autentique, postgres);
    ...
    expect(postgres.query).toHaveBeenCalledWith(expect.stringContaining('from "crm1".deals'), ["d1", "c1"]);
  });
  ...
  it("create rejects invalid contract value", async () => {
    ...
    const postgres = {
      query: jest.fn().mockResolvedValue({ rows: [{ id: "d1", company_id: "c1", deal_valor_contrato: 10 }] }),
    } as any;
    const service = new ContractsService(supabase, { createContract: jest.fn() } as any, postgres);
  ...
  it("download returns signed URL when signed_file_path exists", async () => {
    ...
    const service = new ContractsService({ getAdminClient: jest.fn(() => admin as any) } as any, {} as any, {} as any);
  ...
  it("download throws NotFoundError when no signed contract is available", async () => {
    const contractQuery = createQuery({ data: { id: "ct1", company_id: "c1", contract_data: {} }, error: null });
