...
import { CenturionEditor } from "@/modules/centurions/components/centurion-editor";
...
export default function CenturionEditorPage() {
  const router = useRouter();
  const params = useParams<{ id: string }>();
  const searchParams = useSearchParams();
  const centurionId = params?.id;
  ...
  const backHref = selectedCompanyId ? `/centurions?company_id=${encodeURIComponent(selectedCompanyId)}` : "/centurions";
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>Editor completo (prompt, regras, tools, MCP, follow-ups e playground).</CardDescription>
        </CardHeader>
      ...
      {selectedCompanyId && centurionId ? <CenturionEditor companyId={selectedCompanyId} centurionId={centurionId} /> : null}
    </div>
  );
}
