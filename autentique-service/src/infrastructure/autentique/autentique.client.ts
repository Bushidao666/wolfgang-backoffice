...
export class AutentiqueClient {
  private readonly config: AutentiqueConfig;

  constructor(private readonly configService: ConfigService) {
    this.config = this.configService.get<AutentiqueConfig>("autentique") ?? {
  ...
  private authHeader(): string {
    if (!this.config.apiKey) {
      throw new Error("AUTENTIQUE_API_KEY is required");
    }
  ...
  async createSignatureLink(publicId: string): Promise<string> {
    ...
    const json = await this.graphql<{ createLinkToSignature: { short_link: string } }>(query, {});
  ...
  async getDocument(documentId: string): Promise<unknown> {
    ...
    const json = await this.graphql<{ document: unknown }>(query, {});
    return (json as any).document;
  }

  async listSigners(documentId: string): Promise<unknown[]> {
    const doc = (await this.getDocument(documentId)) as any;
    const signatures = doc?.signatures;
    return Array.isArray(signatures) ? signatures : [];
  }

  async downloadSignedFile(signedUrl: string): Promise<Buffer> {
