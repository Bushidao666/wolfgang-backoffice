-- Hotfix: avoid PL/pgSQL variable/column ambiguity for `provider` inside `fn_create_company_full`.
-- Symptom: 42702 "column reference \"provider\" is ambiguous" during POST /companies when inserting bindings.
...
begin
  ...
  if p_integrations is not null and jsonb_typeof(p_integrations) = 'array' then
    for it in select * from jsonb_array_elements(p_integrations) loop
      v_provider := lower(coalesce(it->>'provider', ''));
      ...
      if mode = 'global' then
        if (it ? 'credential_set_id') and nullif(btrim(it->>'credential_set_id'), '') is not null then
          ...
          raise exception 'credential_set_id is required when mode=global (provider=%)', v_provider;
        end if;
      end if;
      ...
      values (
        v_company.id,
        v_provider::core.integration_provider,
        mode,
        set_id,
        cfg,
        secrets_enc,
